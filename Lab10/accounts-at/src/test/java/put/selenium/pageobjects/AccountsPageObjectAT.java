...
public class AccountsPageObjectAT {
    ...
    public void successfulUserRegistration() throws Exception {
        assertTrue(mainMenu.isOnMainPage());

        mainMenu.clickRegistration();
        assertTrue(registrationPage.isOnRegistrationPage());

        registrationPage.register("user", "password", "Jan Kowalski", "ul. Nowa 10");
        assertTrue(loginPage.isOnLoginPage());

        loginPage.login("user", "password");
        assertTrue(mainPage.isOnMainPage());

        String userInfo = loggedInUserInfo.getUserInfo();
        assertEquals("Jan Kowalski, ul. Nowa 10", userInfo);
    }
